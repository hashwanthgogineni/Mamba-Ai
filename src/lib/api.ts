...
export interface ClarifyResponse {
  ...
  options: { id: string; label: string; example: string; feel: string }[];
}
...
class ApiClient {
  ...
  /**
   * Classify the prompt and return any questions worth asking BEFORE building.
   * Cheap and fast: no project is created and no game is generated by this call.
   */
  async clarifyGame(prompt: string): Promise<ClarifyResponse> {
    const headers = await this.getAuthHeaders();

    const response = await fetch(`${this.baseUrl}/api/v1/generate/clarify`, {
      method: 'POST',
      headers,
    ...
    if (!response.ok) {
      // Never block the build on a failed classification.
      return { genre: '', confidence: 'low', questions: [], options: [] };
    }
  ...
  /**
   * Apply a change to an EXISTING game. Reuses the project's files and id
   * rather than generating a new game from scratch.
   */
  async iterateGame(projectId: string, prompt: string): Promise<GenerateGameResponse> {
    const headers = await this.getAuthHeaders();

    const response = await fetch(`${this.baseUrl}/api/v1/generate/iterate`, {
      method: 'POST',
      headers,
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ detail: 'Failed to apply change' }));
      throw new Error(error.detail || 'Failed to apply change');
    }
